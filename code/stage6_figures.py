...
S3 = ROOT / "longitudinal_stage3/source_data"
S4 = ROOT / "longitudinal_stage3/04_source_data"
REVISION = ROOT / "stage6_submission/06_02_revision"
OUT = REVISION / "06_figures/main"
SOURCE_OUT = REVISION / "06_figures/source_data"
SUPP_OUT = REVISION / "04_supplementary/figures"
...
COHORT_INFO = pd.DataFrame([
    ["GSE236713", "Adult sepsis", "PBL", "Agilent", 106, 93, 0, "Prespecified validation"],
    ["GSE57065", "Adult septic shock", "Whole blood", "Affymetrix", 28, 28, 26, "Pilot"],
    ["GSE95233", "Adult septic shock", "Whole blood", "Affymetrix", 51, 20, 31, "Prespecified validation"],
    ["GSE54514", "Adult sepsis", "Whole blood", "Illumina", 31, 31, 28, "Pilot"],
    ["GSE110487", "Adult septic shock", "Whole blood", "RNA-seq", 31, 0, 31, "Prespecified validation"],
    ["GSE8121", "Paediatric septic shock", "Whole blood", "Affymetrix", 30, 0, 30, "Prespecified validation"],
...
def public_labels(data: pd.DataFrame) -> pd.DataFrame:
    ...
    return data.replace({
        "BLINDED_VALIDATION": "PRESPECIFIED_VALIDATION",
        "BLINDED_VALIDATION_ONLY": "PRESPECIFIED_VALIDATION_ONLY",
        "Blinded validation": "Prespecified validation",
    })
...
def figure1() -> None:
    ...
    box(ax, 0.84, 0.62, 0.15, 0.22, "8 fixed\nsignatures", "#FFF3D5", fs=9.2)
    ...
    box(ax, 0.375, 0.20, 0.25, 0.20, "Primary within-patient windows\nT24: n=172  |  T48: n=146", "#EFF4F7")
    box(ax, 0.71, 0.20, 0.25, 0.20, "Meta-analysis, exact gene\ndecomposition and annotation", "#EFF4F7")
    for x1, x2 in [(0.29, 0.375), (0.625, 0.71)]:
        ax.annotate("", xy=(x2, 0.30), xytext=(x1, 0.30), arrowprops=dict(arrowstyle="->", color="#64737D", lw=1.1))
    ax.text(0.01, 0.97, "Frozen longitudinal design", fontsize=11, fontweight="bold", va="top")
    ax.text(0.01, 0.04, "T24 = 12–36 h; T48 = 36–60 h relative to each study's locked baseline definition.", fontsize=6.8, color="#5F6B76")
...
def figure3() -> None:
    ...
    for idx, (window, title) in enumerate([("T1", "T24 cohort-specific mean delta Z"), ("T2", "T48 cohort-specific mean delta Z")]):
        ...
        heat(ax, sub.to_numpy(float), COHORTS, SIGS, vmin=-1.6, vmax=1.6, cmap="RdBu_r", fmt=".2f", title=title, cbar_label="Mean delta Z")
    ...
    bars = ax3.barh(y, p.I2_percent, height=0.58, color=[COL["purple"] if v >= 50 else COL["sky"] for v in p.I2_percent])
...
def figure4() -> None:
    ...
    arch = pd.read_csv(S4 / "04_08_signature_drift_architecture.csv").set_index("signature_id").reindex(SIGS).reset_index()
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(7.2, 5.1), gridspec_kw={"width_ratios": [1.28, 1], "wspace": 0.44})
    y = np.arange(len(SIGS))[::-1]
    for yi, sig in zip(y, SIGS):
    ...
    for row in arch.itertuples(index=False):
        color = ARCH_COLORS.get(row.drift_architecture, COL["grey"])
        size = 55 + 80 * min(row.median_patient_absolute_contribution_sum, 2.5)/2.5
        ax2.scatter(row.median_patient_dominance_ratio, row.median_patient_cancellation_index, s=size, color=color, edgecolor="white", linewidth=0.7)
        dx, dy = {
    ...
    ax2.axvline(0.60, color="#AAAAAA", lw=0.8, ls="--"); ax2.axhline(0.50, color="#AAAAAA", lw=0.8, ls="--")
    ...
    ax2.set_title("Frozen architecture classification", loc="left", fontweight="bold")
    panel(ax2, "b")
    handles = [mpl.lines.Line2D([], [], marker="o", ls="", color=c, label=k.replace("_", " ").title(), markersize=5.5)
               for k, c in ARCH_COLORS.items() if k in set(arch.drift_architecture)]
    ax2.legend(handles=handles, fontsize=5.3, loc="upper center", bbox_to_anchor=(0.5, -0.19), ncol=1)
    fig.text(0.01, 0.01, "Contributions exactly reconstruct each fixed score change but do not identify causal genes. Point size in panel b encodes median absolute contribution sum.", fontsize=6.2, color="#5F6B76")
    fig.subplots_adjust(bottom=0.25, top=0.91)
    fig.suptitle("Gene-level architecture of T48 score change", x=0.02, y=0.98, ha="left", fontsize=12, fontweight="bold")
    export(fig, "Figure_4_gene_architecture")
...
def draw_pathway_heatmap(ax, coupling: pd.DataFrame):
    ...
    ax.set_xticklabels([SHORT_PATH[p] for p in PATH_ORDER], rotation=40, ha="right", fontsize=7.5)
    ax.set_yticks(range(len(SIGS))); ax.set_yticklabels(SIGS, fontsize=7.6)
    ax.tick_params(length=0)
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            ...
            ax.text(j, i, f"{matrix[i, j]:.2f}", ha="center", va="center", fontsize=6.7, color=color)
...
def figure5() -> None:
    ...
    fig = plt.figure(figsize=(7.2, 7.0))
    gs = fig.add_gridspec(2, 2, height_ratios=[1.0, 1.45], width_ratios=[1.0, 1.15], hspace=0.52, wspace=0.44)
    ...
    for lineage in lineages:
        ...
        ax2.barh(y, vals, left=left, height=0.68, color=LINEAGE_COLORS[lineage], label=lineage.replace("_", " ").title())
        left += vals
    ...
    ax3.set_title("Prespecified pathway coupling at T48", loc="left", fontweight="bold"); panel(ax3, "c")
    fig.text(0.01, 0.005, "Cell sources are HPA-based potential sources; bulk RNA cannot separate abundance from within-cell expression. Gold outline denotes BH-FDR < 0.05.", fontsize=6.6, color="#555555")
    fig.subplots_adjust(bottom=0.19, top=0.97)
...
def figure6() -> None:
    ...
    for sig in SIGS:
        ...
        color = ARCH_COLORS.get(arch.loc[sig, "drift_architecture"], COL["grey"])
        ax.plot(x, vals, marker="o", lw=1.7, ms=4.2, color=color, alpha=0.95)
        label_dy = {"SIG001": -7, "SIG002": 2, "SIG003": -12, "SIG004": 2, "SIG022": 2, "SIG023": 7, "SIG033": -7, "SIG034": -2}[sig]
    ...
    ax.set_title("The apparent signature state depends on sampling time", loc="left", fontweight="bold")

    ax2 = fig.add_subplot(gs[1]); ax2.set_axis_off(); panel(ax2, "b")
    ax2.text(0, 0.98, "Temporal measurement-context framework", transform=ax2.transAxes, va="top", fontweight="bold")
    items = [
        ("Formula", "Preserve the published gene set\nand coefficients"),
        ("Baseline", "Anchor change to the source study's\nlocked initial sample"),
        ("Sampling\nwindow", "Report T24 and T48 separately,\nnot as interchangeable follow-up"),
        ("Cohort\ntransport", "Interpret pooled means with confidence\nand prediction intervals"),
        ("Architecture", "Distinguish dominance, multigene drift,\ncancellation, and cohort dependence"),
        ("Claim\nboundary", "Do not infer monitoring, prognosis,\ntreatment response, or mechanism"),
    ]
    y0 = 0.84
    for i, (head, body) in enumerate(items):
        y = y0 - i*0.14
        ax2.add_patch(FancyBboxPatch((0.00, y-0.075), 0.98, 0.105, boxstyle="round,pad=0.012,rounding_size=0.015",
                                     facecolor="#F2F6F8" if i < 5 else "#FFF3D5", edgecolor="#CFD9DF", linewidth=0.7,
                                     transform=ax2.transAxes))
        ax2.text(0.03, y, head, transform=ax2.transAxes, va="center", fontweight="bold", fontsize=6.8, linespacing=1.0)
        ax2.text(0.46, y, body, transform=ax2.transAxes, va="center", fontsize=5.9, linespacing=1.15)
    fig.suptitle("Sampling time is part of the context for fixed transcriptomic scores", x=0.02, y=0.985, ha="left", fontsize=12, fontweight="bold")
    export(fig, "Figure_6_temporal_measurement_context")
    m.reset_index().to_csv(SOURCE_OUT / "Figure_6_source_data.csv", index=False)
    ...
    for window in ["T1", "T2"]:
        for sig in SIGS:
            ...
            validation = meta[(meta.signature_id == sig) & (meta.time_window == window) &
                              meta.analysis_set.isin(["BLINDED_VALIDATION_ONLY", "PRESPECIFIED_VALIDATION_ONLY"])]
...
def copy_unchanged_supplementary_figures() -> None:
    source_dir = ROOT / "stage5_manuscript/06_supplementary/figures"
...
def main() -> None:
    ...
    supplementary_figure_1(); copy_unchanged_supplementary_figures()
    manifest = {
        "freeze_id": "STAGE6-SCIREP-FORMAT-REVISION-20260717-v1.0",
        "scientific_source": "Frozen Stage 3/4 result tables; no recomputation of estimands",
