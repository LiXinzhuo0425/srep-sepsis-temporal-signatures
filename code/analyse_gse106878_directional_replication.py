#!/usr/bin/env python3
"""Post-freeze directional-replication analysis for GSE106878.
...
def main() -> None:
    ...
    effects["analysis_note"] = (
        "Treatment-arm estimates are descriptive; no between-arm interaction was tested. "
        "All strata use the signature-specific baseline SD calculated from all 47 baseline samples."
    )
    effects.to_csv(OUTDIR / "GSE106878_postfreeze_directional_replication_effects.csv", index=False)
    ...
    pd.DataFrame(
        [
            {
                "dataset": "GSE106878",
                "decision": "INCLUDE_POST_FREEZE_DIRECTIONAL_REPLICATION",
                ...
                "role": "post-freeze directional replication only; randomized treatment arms retained and reported descriptively; no between-arm interaction tested",
            }
        ]
